<p align="center">
  <a href="./README.md"><b>English</b></a> | <a href="./README_RU.md"><b>Русский</b></a>
</p>

# Hi there, we are BMProjects (BMP) 👋

We are an international independent game development and modding studio. Our mission is to transform the gaming experience, build massive immersive worlds, and set new quality standards in the industry.

By leveraging a **"Living Service"** model, we continuously evolve our projects based on analytics and community feedback, delivering rapid updates and maintaining high operational standards.

---

## Featured Projects

| Project | Status | Platform / Versions | Description |
| :--- | :--- | :--- | :--- |
| **Melody of Utopia** *(coming soon)* | In Active Development | Minecraft 1.21.1 NeoForge | Our flagship project in development for over 10 months. A complete overhaul of the gameplay experience. |
| **[Endless Rise: Remastered](https://github.com/BMProjects-Development/Endless-Rise-Remastered)** | Live & Operating | Minecraft 1.16.5 Forge | A massive hit that reached #1 in the "Hidden Gem" category with **100 000+ downloads**. |
| **[Gloomy Rise](https://github.com/BMProjects-Development/Gloomy-Rise)** | Live & Operating | Minecraft 1.16.5 Forge | TechnoMagical Hardcore and RPG Modpack. |
| **[Sacred Logistics](https://github.com/BMProjects-Development/Sacred-Logistics)** | Live & Operating | Minecraft 1.16.5 Forge | Modpack about Technology progression with unique mechanics. |
| <a href="./PROJECTS_LIST.md"><b>**Other Projects**</b></a> | Planning & Dev | Minecraft & Hytale | We are constantly expanding our ecosystem with new immersive worlds. |

---

## Our Tech Stack

| Category | Technologies & Tools |
| :--- | :--- |
| **Languages** | `Kotlin`, `Java`, `JavaScript` (KubeJS Scripting), `ZenScript` (CraftTweaker Scripting) |
| **Frameworks & APIs** | `Forge API`, `Fabric API`, `NeoForge` |
| **Build Tools** | `Gradle` |
| **Profiling & Opt** | `Spark Profiler` |
| **Data Structures** | `JSON`, `NBT Structures` |

---

## International Team

BMP is a decentralized team of **20+ professionals** spanning across Russia, the CIS and Europe. We bring together developers, designers, writers, and sound engineers working via a transparent remote workflow on GitHub.

---



## Leadership & Core Development

* **Founder & CEO / Lead Developer:** **Maxim** ([@BarMaxx](https://github.com/BarMaxx))  
  *Directing studio operations, project architecture, and core development for over 4.5 years within the Minecraft modding and game development industry.*

---

## Useful Information & Links

### Main Community Hubs
* **[Telegram](https://t.me/BMProjects)** - Our main community hub with the faster news.
* **[Discord](https://discord.gg/9GWKBVw3Ty)** - Our main community hub for chats, updates, and tech support.
* **[Reddit](https://www.reddit.com/r/BMProjects/)** - Our international platform for discussions and community feedback.
* **[VK](https://vk.com/bmprojects)** - Our official VK community for regional news and announcements.

### Media & Content
* **[YouTube](https://www.youtube.com/@TheBMProjects)** - Project showcases, trailers, and development insights.
* **[TikTok](https://www.tiktok.com/@bmprojectsoff)** - Short previews, teasers, and fun moments.

### Project Platforms
* **[CurseForge](https://www.curseforge.com/members/thebarmaxx/projects)** - Download and easily install our projects.
* **[Modrinth](https://modrinth.com/user/BarMaxx)** - A fast and modern platform to access our mods and modpacks.

### Support the Studio
* **[Patreon](https://www.patreon.com/c/BMProjectsMinecraft)** - Subscribe to unlock exclusive perks and support our development.
* **[Boosty](https://boosty.to/barmaxx)** - Subscribe to unlock exclusive perks and support our development (Regional).

### Other
* **Email** = `bmpland.main@gmail.com` or `bmpland.main@mail.ru`

---
<p align="center">
  <i>Building the future of gaming, step by step.</i>
</p>
